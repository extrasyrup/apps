...
function runApi(c, type) {
    if(c >= apiMaxPages) {
        if(finalResult.length)
            writeFile(`data/greenwood/places-${type}-${getRnd(999)}.csv`, finalResult); //Save all results to csv file  OLD//writeFile(`data/places-${type}-${getRnd(999)}.json`, JSON.stringify(finalResult)); //Save all results to json file
